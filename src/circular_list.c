...
{
    number++;
    new_circular_list = (n *)malloc(sizeof(n));
    new_circular_list->val = info;
    new_circular_list->next = NULL;
    new_circular_list->prev = NULL;
...
void add_node()
{
    ...
    scanf("%d", &info);
    new_circular_list = create_node(info);
 
    if (first == last && first == NULL)
    {
 
        first = last = new_circular_list;
        first->next = last->next = NULL;
        first->prev = last->prev = NULL;
    }
    else
    {
        last->next = new_circular_list;
        new_circular_list->prev = last;
        last = new_circular_list;
        last->next = first;
        first->prev = last;
    }
}
...
void insert_at_first()
{
    ...
    {
        new_circular_list->next = first;
        first->prev = new_circular_list;
        first = new_circular_list;
        first->prev = last;
        last->next = first;
    ...
    else
    {
        ...
        else
        {
            for (ptr = first, i = 1;i <= number;i++)
            {
                ...
                if (i == pos-1)
                {
                    prevnode->next = new_circular_list;
                    new_circular_list->prev = prevnode;
                    new_circular_list->next = ptr;
                    ptr->prev = new_circular_list;
